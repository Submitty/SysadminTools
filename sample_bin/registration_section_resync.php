...
class registration_section_resync {
	...
	private static $db_master_conn;
	private static $db_course_conn;
	...
	public function __destruct() {
		if (pg_connection_status(self::$db_master_conn) !== pgsql_connection_ok) {
			pg_close(self::$db_master_conn);
		}

		if (pg_connection_status(self::$db_course_conn) !== PGSQL_CONNECTION_OK) {
			pg_close(self::$db_course_conn);
		}
	}
	...
	private function init() {
		...
		self::$db_master_conn = pg_connect(sprintf("dbname=submitty host=%s user=%s password=%s sslmode=prefer", self::$db_host, self::$db_user, self::$db_password));
		if (pg_connection_status(self::$db_master_conn) !== PGSQL_CONNECTION_OK) {
	...
	private function process() {
		...
		foreach (self::$academic_terms as $term) {
			...
			$res = pg_query_params(self::$db_master_conn, "SELECT course FROM courses WHERE semester = $1", array($term));
			if ($res === false) {
			...
			if (count($courses) < 1) {
				fprintf(STDERR, "No courses registered for term code '%s'.%s", $term, PHP_EOL);
				continue;
			}
			...
			foreach($courses as $course) {
				...
				self::$db_course_conn = pg_connect(sprintf("dbname=%s host=%s user=%s password=%s sslmode=prefer", $db_name, self::$db_host, self::$db_user, self::$db_password));
				if (pg_connection_status(self::$db_course_conn) !== PGSQL_CONNECTION_OK) {
